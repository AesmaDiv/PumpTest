...
def draw_charts():
    if vars.graph_info:
        vars.graph_info.clearCharts()
        if vars.dictType:
            charts = load_charts()
            for chart in charts.values():
                vars.graph_info.addChart(chart, chart.getName())
            vars.graph_info.setLimits(vars.dictType['Min'],
                                      vars.dictType['Nom'],
                                      vars.dictType['Max'])
        display_charts(vars.wnd_main.frameGraphInfo)


def display_charts(frame: QFrame):
    vars.graph_info.renderToImage(frame.size())
    frame.setStyleSheet("QFrame { background-image: url('" + vars.path_to_pix + "'); }")
...
def get_points(of='ethalon'):
    ...
    source = vars.dictType if is_ethalon else vars.dictTest
